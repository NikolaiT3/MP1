...
void Init ( int memSize, int blockSize, int tierSize ); // initializes the linked list, should be called once from the main
void freeReset ();
...
char* Lookup ( int key, int index );	// Looks up the first item with the given and returns a pointer to the value portion (the value length and the actual value) 
